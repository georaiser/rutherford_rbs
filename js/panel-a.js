 ...
 * Inset: imagen 3D del modelo atomico de Rutherford.
 *
...
const panelA = (() => {
  ...
  // Parametros de impacto fisicos (fm). Al variar E, a0 cambia -> distintos angulos.
  const B_PHYS_FM = [5, 12, 26, 55, 95, 155];
  const B_COLORS  = ['#ef4444','#f97316','#facc15','#4ade80','#38bdf8','#a78bfa'];

  // Posicion del nucleo en el canvas
  const NUC_X   = W * 0.44;
  const NUC_Y   = H * 0.50;
  const R_START = 32;
  const SCALE   = NUC_X / (R_START + 2); // px/a0

  // Trayectorias + fases de animacion
  let trajs  = [];
  let phases = [];
  const SPEED = 0.006;

  // Imagen del modelo atomico (inset)
  let atomImg = null;
  let imgLoaded = false;

  // Particulas del modelo de Thomson (generadas una sola vez)
  let thomsonParts = [];
  ...
  function drawAtomicModelInset() {
    ...
    if (imgLoaded && atomImg) {
      ctx.save();
      ctx.beginPath();
      ctx.roundRect(IX, IY, IW, IH, 5);
      ctx.clip();
      ctx.drawImage(atomImg, IX, IY, IW, IH);
      ctx.restore();
    } else {
      // Fallback si imagen no cargo
  ...
  function drawRutherford() {
    ...
    // Trayectorias (simetria arriba/abajo)
    ...
    // Escala (5 a0) — esquina inferior izquierda, lejos del inset
    const a0  = Physics.calcA0(E_mev);
    ...
    // Leyenda b -> angulo: posicionada arriba a la derecha, termina en y~104 (inset empieza en y=192)
    let ly = 14;
  ...
  function drawThomson() {
    ...
    // Electrones (azul) y protones (rojo) embebidos
    ...
    ctx.textAlign = 'center';
    ctx.fillText('\u00e1tomo Au \u2014 radio \u2248 1.45 \u212b', ax, ay - Rpx - 10);

    // Trayectorias Thomson (lineas casi rectas - fisica real)
    const a0   = Physics.calcA0(E_mev);
    ...
    // Nucleo (punto sub-pixel - ese ES el mensaje)
    ...
    ctx.textAlign = 'right';
    ctx.fillText('1.45 \u212b \u2190 radio at\u00f3mico', W - 10, H - 4);

    // Angulo Thomson maximo
    const thMaxDeg = Physics.thetaThomsonMax_deg(a0);
    ctx.font = '11px Inter, sans-serif'; ctx.fillStyle = 'rgba(200,200,255,0.50)';
    ctx.textAlign = 'left';
    ctx.fillText(
      '\u03b8\u2098\u2090\u02e3 Thomson = ' + (thMaxDeg * 1000).toFixed(2) + ' \u00d7 10\u207b\u00b3 \u00b0 \u2014 sub-p\u00edxel a esta escala',
      14, H - 10
    );
  ...
  function setMode(m) {
    ...
    // Textos estaticos: solo alternar visibilidad, nunca reescribir innerHTML
    ...
    // Re-renderizar MathJax si esta disponible (los bloques recien mostrados
    // pueden tener formulas que aun no se procesaron)
  ...
  function init() {
    ...
    // Precargar imagen del modelo atomico
    atomImg = new Image();
